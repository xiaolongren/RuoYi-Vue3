<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list.slice((pageNum - 1) * pageSize, pageNum * pageSize)" style="width: 100%;">
      ...
      <el-table-column label="培训" align="center" prop="project" :show-overflow-tooltip="true" />
    ...
    <el-dialog v-model="dialogVisible" title="审核培训经历" :show-close="true" @close="closeDialog">
      ...
      <div>
        培训开始时间  {{ mformatTimestamp(this.currentRow.startTime) }}
      </div>
      <div>
        培训结束时间  {{ mformatTimestamp(this.currentRow.endTime) }}
      </div>
      <div>
       培训时长   {{this.currentRow.duration}}
      </div>
      <p></p>
...
<script>
import { getWaiting4ReviewTraining, verifyFail, trainingverifySuccess,trainingverifyFail } from "@/api/compliance/listener";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getWaiting4ReviewTraining().then(response => {
        this.loading = false;
    ...
    mverifySuccess() {
      ...
      trainingverifySuccess(this.currentRow.idStr,this.global.uid,this.global.token).then(value => {
        if (value.errorCode == 0) {
    ...
    mrefuseRequestListener() {
      this.$prompt('请输入未通过审核意见', '审核未通过', {
        ...
        trainingverifyFail(this.currentRow.idStr, value,this.global.uid,this.global.token).then((value) => {
          if (value['errorCode'] == 0) {
            let index = this.list.indexOf(this.currentRow); // 查找元素的索引
            if (index !== -1) {
              this.list.splice(index, 1); // 从索引位置删除1个元素
